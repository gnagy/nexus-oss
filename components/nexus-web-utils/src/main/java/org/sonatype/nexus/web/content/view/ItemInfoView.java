...
/**
 * Info view provider, revealing simple informations about the item.
 * <p>
    ...
    extends ViewSupport
{
  ...
  @Inject
  public ItemInfoView(final Renderer renderer) {
    super(renderer);
  }
  ...
  public void renderView(final HttpServletRequest request, final HttpServletResponse response,
      ...
      throws IOException
  {
    // this view must have access to file item, 404 if not
    if (item instanceof StorageFileItem) {
      // TODO: re-implement this, current one uses DTOs from restlet1x (needs UI changes)
      // For now, to make UI work, redirect this
      // http://localhost:8081/nexus/content/repositories/central/log4j/log4j/1.2.13/log4j-1.2.13.pom?describe=info
      // to
      // http://localhost:8081/nexus/service/local/repositories/central/content/log4j/log4j/1.2.13/log4j-1.2.13.pom?describe=info
